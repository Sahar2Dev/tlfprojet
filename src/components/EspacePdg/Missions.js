...
setIdcongeSupp(ids)
  ...
  const ValiderConge =async (id,iduser,datedebut,datefin,validation,validationrh) => {
    ...
    if (idchef==iduserinfo){
        ...
        if (validationn!=1){
    // sendMail(emailchef,"IPS Time:  Avis Favorable pour la demande du mission "+user_name + "   " ,"Bonjour, La demande de mission du collaborateur "+user_name + "   " +"  du "+datedebut+" au "+datefin+"  est validé avec un avis favorable !")

      sendMail(emailemploye,"IPS Time: Avis Favorable pour la demande du mission ","Bonjour "+user_name+" "+",  Votre demande de missions du"+datedebut+" au "+datefin+" a été validée par un avis favorable officiel !")
     
      JSON.parse(emailsDRHS).map(x=>
        x.id==iduserr?"":   sendMail(x.email,"IPS Time:  Avis Favorable par chef pour la demande du mission "+user_name + "   " ,"Bonjour, La demande de mission du collaborateur "+user_name + "   " + "  du "+datedebut+" au "+datefin+"  est validé avec un avis favorable par chef!")
...
if (idchef==iduserinfo){
   ...
   //   sendMail(emailchef,"IPS Time:  Avis défavorable pour la demande du mission "+user_name + "   "  ,"Bonjour, La demande de mission du collaborateur "+user_name + "   " + "  du "+datedebut+" au "+datefin+"  est refusé avec un avis défavorable !")

    sendMail(emailemploye,"IPS Time: Avis défavorable pour la demande du mission du "+datedebut+" au "+datefin," Bonjour "+user_name+"&nbsp;"+"  La demande de missions du  "+datedebut+" au "+datefin+"   est refusé avec un avis défavorable officiel !")
    JSON.parse(emailsDRHS).map(x=>
      x.id==iduserr?"":    sendMail(x.email,"IPS Time:  Avis défavorable  par chef pour la demande du mission "+user_name + "   " ,"Bonjour, La demande de mission du collaborateur "+user_name + "   " + "  du "+datedebut+" au "+datefin+"  est refusé avec un avis défavorable par chef!")
...
const onClickAnnuler = (id, iduserr, datedebut, datefin, validationn, validationrhh, idchef, idrh, emailemploye, emailchef, user_name, last_name) => {
  ...
  if (idchef == iduserinfo) {
   ...
   //  sendMail(emailchef,"IPS Time:  Avis défavorable pour la demande du congé "+user_name  ,"Bonjour, La demande de congé du collaborateur "+user_name + "   " +"  du "+datedebut+" au "+datefin+"  est refusé avec un avis défavorable !")
      ...
      JSON.parse(emailsDRHS).map(x=>
        x.id==iduserr?"":    sendMail(x.email,"IPS Time:  Avis défavorable par chef pour la demande du congé  "+user_name ," Bonjour, La demande de congé du collaborateur "+user_name + "   " +"  du "+datedebut+" au "+datefin+"  est annulé avec un avis défavorable par chef !")
      ...
      if (validationrhh == '') {
        const validationrh = 0
        return Annulermission(id, iduserr, datedebut, datefin, validationAnu,validationrh)
...
const Annulermission = (id, iduser, datedebut, datefin, validation,validationAnu, validationrh) => {
  ...
  fetch(url+'RefusConge/' + id, {
    ...
    Mouchard("encours", "annulé", iduser,iduserinfo, "Refus de congé de " + datedebut + "au " + datefin)
...
<>
   ...
   Valider
        ...
        <DialogActions>
          ...
          <td>
          {c.show == "true" ?  <a className='btn-sm btn-danger' onClick={()=>{handleClickOpenrefus(c.idconge,c.iduser,c.datedebut,c.datefin,c.validation,c.validationrh,c.chef_id,c.rh_id,c.emailemploye,c.email_chef,c.user_name,c.last_name)}}>
       ...
       Refuser
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            êtes-vous sûr de vouloir refuser cette mission ?{idcongerefus}
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleCloserefus}>non</Button>
